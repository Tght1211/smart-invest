...
> **自主经营纪律（用户已授权全自动记账，会完全照指令买卖）**：
>
> - **只有盘尾 14:48 这一封负责"执行 + 记账"**。开盘/盘中两封只发信息和预告，**绝不写 DB**。
...
>   4. 卡片黄框写明已自动记账，如"已为你记账：支付宝搜 512480 买入「半导体ETF国联安」¥2000"。
...
> - **物理边界**：你仍无法替用户在支付宝真正点"买入"；黄框指令供用户照抄执行。自动记账是基于"用户会完全跟单"的授权前提。

...
今日估算盈亏（尾盘 14:30）
+57.53元
持仓市值 ¥21,845 | 昨日 +329.93 | 累计 +¥3,065 | +16.32%
:::
```
...
**③ 持仓表格** — 表头固定 7 列。`今日`=当日估算%，`今日盈亏`=今日预估收益(元)，`持有收益`=累计浮盈(元，=（现价−成本)×份额)，`累计`=持有收益率%，`市值`=当前市值，`天数`=持有天数

```
| 基金 | 今日 | 今日盈亏 | 持有收益 | 累计 | 市值 | 天数 |
|------|------|---------|---------|------|------|------|
| 广发纳斯达克100C | -0.09% | -13.12 | +3,761.00 | +23.67% | 19,651 | 117天 |
```

> 收益口径：**持仓收益率 = 持仓浮盈 ÷ 持仓成本，现金完全不计入**。`card_wallet` 顶部另给 本金（累计投入）/ 持仓成本 / 可用现金（=本金−持仓成本）三件套，别把 5 万本金当成持仓成本，也别把现金算进收益。
...
| 卡片槽位 | 命令 | 取值 |
|---|---|---|
| `:::card` 顶部 | `fetch_fund.py portfolio-check --account 主线` | `total_today_pnl` / 持仓市值 / 持仓累计盈亏 / 持仓收益率%（现金不计入） |
| 持仓表格 | `fetch_fund.py portfolio-check --account 主线` | 每只 `today_pnl`(今日预估) / 持有收益(元) / 累计% / 市值 |
...
每个交易日发三封卡片邮件，**共用同一套卡片结构**，只有顶部数字语义和黄框文案随时段变化。场外基金按当日收盘净值成交、约 15:00 截单，**盘尾 14:30 是真正的下单决策窗口，三封里最重要**。
...
**(C) 盘尾 14:30 — 最终下单指令（最重要）**：顶部用尾盘估算盈亏，黄框写**今天最终怎么买/卖**
...
- **只有盘尾 14:30 这一封负责"下单 + 记账"**。开盘/盘中两封只发信息和预告，**绝不写 DB**。

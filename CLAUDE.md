...
| `scripts/web_panel.py` | Always-on local web dashboard (stdlib `http.server`, no deps). `start`(后台)/`serve`(前台)/`stop`/`status`; PID in `data/web_panel.pid`. Renders the same cards as email by reusing `daily_report.build_context`+`assemble` → `send_email.markdown_to_html` (90s cache, 120s auto-refresh, account/session switcher). `--host 0.0.0.0` exposes to LAN. Conversational triggers「打开面板/关闭面板」. `assemble`/`card_plan` take `persist=False` so panel renders never overwrite the real daily-plan baseline. |
